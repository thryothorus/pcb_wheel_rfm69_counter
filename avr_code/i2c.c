...
//         uint8_t status = i2c_start(address << 1); // Shift address and start
//         communication if (status == 0x18 || status == 0x40) {
//             // If we received an ACK (0x18: Master Transmitter, 0x40: Master
//             Receiver) char buffer[32]; snprintf(buffer, sizeof (buffer),
